...
public class ImageIOTest {
    ...
    public void markImageByMoreTextOrigin() {
        try {
            ...
            Font font = Font.createFont(Font.TRUETYPE_FONT, new File("d:/data/HYKaiTiJ.ttf")).deriveFont(FONT_STYLE, FONT_SIZE);
            ...
            int size = list.size();
            int maxCharSize = 5;
            List<Grid.Node> loc = ThreadLocalRandom.current().ints(0, size).limit(maxCharSize).mapToObj(x -> list.get(x)).collect(Collectors.toList());
            int[] chars = ThreadLocalRandom.current().ints(0, cjk.length).limit(maxCharSize).map(i -> cjk[i]).toArray();
            for (int aChar : chars) {
    ...
    public void drawChar(char c, BufferedImage bufferedImage, Font font, Grid.Node node) {
        ...
        int angle = random.ints(0, 360).limit(1).findFirst().getAsInt();
        ...
        //width比较
        int a = g2d.getFontMetrics().charWidth((int) ('丧' + 1));
        int b = g2d.getFontMetrics().charWidth((int) ('丧' ));
        
        int glyphId = trueTypeFont.getMapper().charToGlyph('修');
        System.out.println("glyphId:"+glyphId);
        
        //gv初始化了字符串glyphCode才有值
        GlyphVector gv = font.createGlyphVector(g2d.getFontRenderContext(), "修");
        int glyphCode = gv.getGlyphCode(0);
        
        System.out.println("" + gv.getGlyphMetrics(0));
        System.out.println("glyphCode:" + glyphCode);
        
        GlyphVector glyphVector = new StandardGlyphVector(font, new int[]{20007,20008}, g2d.getFontRenderContext());
        System.out.println("g2dClassName: " + g2d.getClass().getName());
        System.out.println((char)20007);
        System.out.println((char)20008);
        //丧 丧+1
        GlyphMetrics sang = glyphVector.getGlyphMetrics(0);
        GlyphMetrics sang_plus_1 = glyphVector.getGlyphMetrics(1);
        System.out.println("sang       : " + sang.getAdvanceX());
        System.out.println("sang_plus_1: " + sang_plus_1.getAdvanceX());
        System.out.println("sang       : " + sang.getAdvanceY());
        System.out.println("sang_plus_1: " + sang_plus_1.getAdvanceY());
        System.out.println("sang       : " + sang.getBounds2D());
        System.out.println("sang_plus_1: " + sang_plus_1.getBounds2D());
        */
        GlyphVector glyphVector = new StandardGlyphVector(font, new char[]{c}, g2d.getFontRenderContext());
    ...
    public void drawx(BufferedImage bufferedImage,Font font, float alpha) {
        //创建绘图工具对象
        Graphics2D g2d = bufferedImage.createGraphics();
        //消除文字锯齿
        g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
    ...
    public void drawStringBox(BufferedImage bufferedImage, Font font, int x, int y) {
        ...
        //矩形
        int recWidth = g2d.getFontMetrics().getMaxAdvance();
        int recHeight = g2d.getFontMetrics().getHeight();
        int recDescent = g2d.getFontMetrics().getMaxDescent();
        System.out.println(recWidth + "  " + recHeight);
        
        g2d.drawRect(x, y - recHeight + recDescent, recWidth, recHeight);
        g2d.dispose();
    }
    ...
    public void fontSizeTest() {
        ...
        Graphics2D g2d = bufferedImage.createGraphics();
        //设置图片背景颜色
        //bufferedImage = g2d.getDeviceConfiguration().createCompatibleImage(imgWidth, imgHeight, Transparency.TRANSLUCENT);
        int m = 60;
        for (int i = 8; i < m; i++) {
            Font font = new Font("宋体", Font.PLAIN, i);
            FontMetrics fm = g2d.getFontMetrics(font);
            System.out.println(String.format("ascent:%s, descent:%s, maxasc:%s, maxdesc:%s, width:%s, advWidth:%s, height:%s, topBaseline: %s", fm.getAscent(), fm.getDescent(), fm.getMaxAscent(), fm.getMaxDescent(), fm.stringWidth("碥"), fm.getMaxAdvance(), fm.getHeight(), fm.getHeight() - fm.getDescent()));
        }
    }
    ...
    public void GridTest() {
        ...
        Grid grid = new Grid(325.0f, 200.f, fontSize);
        List<Grid.Node> list = grid.nodes;
        int size = list.size();
        int maxCharSize = 5;
        int chosen = 2;
        List<Grid.Node> s = ThreadLocalRandom
                .current()
                .ints(0, size)
                .limit(maxCharSize)
        ...
        s.forEach(x -> {
            System.out.println(String.format("nox:%s noy:%s", x.no_x, x.no_y));
        });
    ...
    public class Grid {
        ...
        public Grid(float width, float height, int gap) {
            this.width = width;
            this.height = height;
            this.gap = gap;
            this.genNodes();
        ...
        private void genNodes() {
            if (gap > width || gap > height) {
                this.nodes = Collections.unmodifiableList(Collections.emptyList());
            } else {
